...
deviceRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (!serial || !apiKey) throw new HttpError(401, "Device authentication required");
    const deviceResult = await pool.query<{
      id: string; company_id: string; device_type: string; api_key_hash: string;
    }>("SELECT id,company_id,device_type,api_key_hash FROM devices WHERE serial_number=$1 AND active=true", [serial]);
    ...
    try {
      ...
      const identity = await client.query<{ employee_id: string; biometric_type: string }>(
      ...
      const livenessRejected = identity.rows[0]?.biometric_type === "FACE" && input.livenessPassed !== true;
      const inserted = await client.query<{ id: string; punch_id: string | null }>(
        ...
        [device.company_id, device.id, employeeId, input.eventId, input.externalUserId,
          livenessRejected ? "ACCESS_DENIED" : input.eventType, input.occurredAt, input.confidence ?? null,
          input.livenessPassed ?? null, JSON.stringify(input.metadata ?? {})],
      );
      let punchId = inserted.rows[0].punch_id;
      if (!punchId && employeeId && !livenessRejected && ["ENTRY", "EXIT"].includes(input.eventType)) {
        ...
        const punch = await client.query<{ id: string }>(
          ...
          [device.company_id, employeeId, shift.rows[0]?.id ?? null,
            input.eventType === "ENTRY" ? "CLOCK_IN" : "CLOCK_OUT", input.occurredAt,
            device.device_type === "TURNSTILE" ? "TURNSTILE" : "KIOSK", device.id],
        );
        punchId = punch.rows[0].id;
        await client.query("UPDATE access_events SET punch_id=$1 WHERE id=$2", [punchId, inserted.rows[0].id]);
      }
      await client.query("UPDATE devices SET last_seen_at=now() WHERE id=$1", [device.id]);
      await client.query("COMMIT");
      response.status(202).json({ data: { accepted: true, eventId: inserted.rows[0].id, employeeMatched: Boolean(employeeId), punchId, livenessRejected } });
    } catch (error) {
...
export function createDeviceSecret(): string { return randomBytes(32).toString("hex"); }
export function hashDeviceSecret(secret: string): string { return hash(secret); }
