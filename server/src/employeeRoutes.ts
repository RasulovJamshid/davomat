...
import {
  bindOrVerifyMobileDevice,
  decodeAttendanceSelfie,
} from "./mobileVerification.js";
  ...
  asyncHandler(async (request, response) => {
    ...
    try {
      ...
      const employee = await client.query<{
        ...
        user_id: string | null;
        full_name: string;
        access_role: "EMPLOYEE" | "LOCATION_MANAGER" | "ADMINISTRATOR";
      }>(
      ...
      const accountRole =
        employee.rows[0].access_role === "ADMINISTRATOR"
          ? "ADMIN"
          : employee.rows[0].access_role === "LOCATION_MANAGER"
            ? "MANAGER"
            : "EMPLOYEE";
      ...
      if (userId) {
        ...
        if (!updated.rows[0])
          throw new HttpError(404, "Linked user account not found");
      } else {
      ...
      await client.query(
        `UPDATE employees SET user_id=$1,email=lower($2),status=CASE WHEN status='INVITED' THEN 'ACTIVE' ELSE status END,updated_at=now() WHERE id=$3`,
        [userId, input.email, employeeId],
      );
      await client.query(
      ...
      const delivery = await sendEmployeeInvitation({
        email: input.email.toLowerCase(),
        name: employee.rows[0].full_name,
...
employeeRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    const [
      ...
      pool.query(
        `SELECT s.id,s.starts_at AS "startsAt",s.ends_at AS "endsAt",s.unpaid_break_minutes AS "unpaidBreakMinutes",s.live_tracking_enabled AS "liveTrackingEnabled",l.name AS location
      ...
      pool.query(
       ...
       FROM leave_requests WHERE company_id=$1 AND employee_id=$2 ORDER BY starts_on DESC LIMIT 30`,
        [companyId, employeeId],
      ),
      pool.query(
...
const selfPunchSchema = z.object({
  ...
  deviceInstallationId: z.string().min(32).max(160),
  devicePlatform: z.enum(["ANDROID", "IOS"]),
  deviceLabel: z.string().trim().min(2).max(120),
  faceImageBase64: z.string().min(4000).max(2_100_000),
  faceImageMime: z.literal("image/jpeg"),
  faceCapturedAt: z.string().datetime({ offset: true }),
});
  ...
  asyncHandler(async (request, response) => {
    ...
    try {
      ...
      const employee = await client.query<{
        ...
        `SELECT e.id,e.primary_location_id,c.timezone FROM employees e JOIN companies c ON c.id=e.company_id WHERE e.user_id=$1 AND e.company_id=$2 AND e.status='ACTIVE' FOR UPDATE OF e`,
        [sub, companyId],
      );
      if (!employee.rows[0])
        throw new HttpError(
      ...
      const capturedAt = new Date(input.faceCapturedAt);
      if (Math.abs(Date.now() - capturedAt.getTime()) > 3 * 60_000)
        throw new HttpError(
          400,
          "Take a new selfie immediately before recording attendance",
        );
      const faceImage = decodeAttendanceSelfie(input.faceImageBase64);
      const deviceBindingId = await bindOrVerifyMobileDevice(client, {
        companyId,
        userId: sub,
      ...
      if (!allowedNext[previousType]?.includes(input.eventType))
        throw new HttpError(
          409,
          `Cannot record ${input.eventType.toLowerCase().replaceAll("_", " ")} after ${previousType.toLowerCase().replaceAll("_", " ")}`,
        );
      ...
      const shift = await client.query<{
       ...
       AND (starts_at AT TIME ZONE $3)::date=(now() AT TIME ZONE $3)::date ORDER BY abs(extract(epoch FROM starts_at-now())) LIMIT 1`,
        [companyId, employee.rows[0].id, employee.rows[0].timezone],
      );
      const locationId =
        shift.rows[0]?.location_id ?? employee.rows[0].primary_location_id;
      const location = locationId
        ? await client.query<{
            ...
            `SELECT name,latitude,longitude,geofence_radius_m FROM locations WHERE id=$1 AND company_id=$2 AND active=true`,
            [locationId, companyId],
          )
        : null;
      const configured =
        location?.rows[0]?.latitude != null &&
        location.rows[0].longitude != null;
      const hasCoordinates = input.latitude != null && input.longitude != null;
      const withinGeofence =
        configured && hasCoordinates
          ? isWithinGeofence(
              {
                latitude: Number(location!.rows[0].latitude),
                longitude: Number(location!.rows[0].longitude),
              },
              { latitude: input.latitude!, longitude: input.longitude! },
              location!.rows[0].geofence_radius_m,
            )
          : null;
      ...
      await client.query(
        `INSERT INTO attendance_face_verifications(punch_id,company_id,employee_id,device_binding_id,image_data,image_mime,image_sha256,captured_at)
       VALUES($1,$2,$3,$4,$5,'image/jpeg',$6,$7)`,
        [
          ...
          deviceBindingId,
          faceImage,
          createHash("sha256").update(faceImage).digest("hex"),
          input.faceCapturedAt,
        ],
      );
      if (
        ...
        const type =
          withinGeofence === false ? "OUTSIDE_GEOFENCE" : "LOCATION_UNVERIFIED";
        const severity = withinGeofence === false ? "HIGH" : "MEDIUM";
        const details =
          withinGeofence === false
            ? `Clock-in was outside ${location!.rows[0].name}`
            : "Clock-in location could not be verified";
        await client.query(
      ...
      await client.query(
        `INSERT INTO audit_logs(company_id,actor_user_id,action,entity_type,entity_id,next_value)
       VALUES($1,$2,'EMPLOYEE_PUNCH_CREATED','PUNCH',$3,$4)`,
...
employeeRouter.post(
  "/me/live-location",
  asyncHandler(async (request, response) => {
    const input = liveLocationSchema.parse(request.body);
    const { sub, companyId, role } = (request as AuthRequest).auth;
    if (role !== "EMPLOYEE")
      throw new HttpError(403, "Employee access required");
    const capturedAt = new Date(input.capturedAt);
    if (Math.abs(Date.now() - capturedAt.getTime()) > 2 * 60_000)
      throw new HttpError(400, "Live location update is no longer current");
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
      const employee = await client.query<{ id: string }>(
        `SELECT id FROM employees WHERE user_id=$1 AND company_id=$2 AND status='ACTIVE' FOR UPDATE`,
        [sub, companyId],
      );
      if (!employee.rows[0])
        throw new HttpError(403, "An active employee profile is required");
      const shift = await client.query<{ id: string; ends_at: Date }>(
        `SELECT id,ends_at FROM shifts WHERE company_id=$1 AND employee_id=$2 AND status='PUBLISHED' AND live_tracking_enabled=true AND now() BETWEEN starts_at AND ends_at AND $3::timestamptz BETWEEN starts_at AND ends_at ORDER BY starts_at DESC LIMIT 1 FOR UPDATE`,
      ...
      await client.query(
        `INSERT INTO live_location_updates(shift_id,company_id,employee_id,device_binding_id,latitude,longitude,accuracy_m,captured_at)
         VALUES($1,$2,$3,$4,$5,$6,$7,$8)
         ON CONFLICT(shift_id) DO UPDATE SET device_binding_id=excluded.device_binding_id,latitude=excluded.latitude,longitude=excluded.longitude,accuracy_m=excluded.accuracy_m,captured_at=excluded.captured_at,received_at=now()
         WHERE live_location_updates.captured_at <= excluded.captured_at`,
        [
          shift.rows[0].id,
          companyId,
          employee.rows[0].id,
...
employeeRouter.get(
  ...
  asyncHandler(async (request, response) => {
    ...
    await pool.query(
      `DELETE FROM live_location_updates u USING shifts s WHERE u.shift_id=s.id AND u.company_id=$1 AND (s.status<>'PUBLISHED' OR s.live_tracking_enabled=false OR now()>s.ends_at)`,
      [companyId],
    );
...
employeeRouter.get(
  "/employees/:id/mobile-device",
  requireManager,
  asyncHandler(async (request, response) => {
    const employeeId = z.string().uuid().parse(request.params.id);
    const { companyId } = (request as AuthRequest).auth;
    const result = await pool.query(
      `SELECT b.id,b.platform,b.device_label AS "deviceLabel",b.bound_at AS "boundAt",b.last_seen_at AS "lastSeenAt" FROM mobile_device_bindings b JOIN employees e ON e.id=b.employee_id WHERE b.employee_id=$1 AND b.company_id=$2 AND e.company_id=$2 AND b.revoked_at IS NULL`,
      [employeeId, companyId],
    );
...
employeeRouter.delete(
  ...
  asyncHandler(async (request, response) => {
    ...
    const { companyId, sub } = (request as AuthRequest).auth;
    const result = await pool.query(
      `UPDATE mobile_device_bindings SET revoked_at=now(),reset_by=$1 WHERE employee_id=$2 AND company_id=$3 AND revoked_at IS NULL RETURNING id`,
      [sub, employeeId, companyId],
    );
    if (!result.rows[0])
      throw new HttpError(404, "No mobile device is linked to this employee");
    await pool.query(
      `INSERT INTO audit_logs(company_id,actor_user_id,action,entity_type,entity_id,next_value) VALUES($1,$2,'MOBILE_DEVICE_RESET','EMPLOYEE',$3,$4)`,
      [
        companyId,
        sub,
        employeeId,
        JSON.stringify({ bindingId: result.rows[0].id }),
      ],
    );
...
employeeRouter.get(
  "/punches/:id/verification-photo",
  requireManager,
  asyncHandler(async (request, response) => {
    const punchId = z.string().uuid().parse(request.params.id);
    const { companyId } = (request as AuthRequest).auth;
    const result = await pool.query<{ image_data: Buffer; image_mime: string }>(
      `SELECT image_data,image_mime FROM attendance_face_verifications WHERE punch_id=$1 AND company_id=$2`,
      [punchId, companyId],
    );
    if (!result.rows[0])
      throw new HttpError(404, "Attendance selfie not found");
    response.setHeader("Content-Type", result.rows[0].image_mime);
    response.setHeader("Cache-Control", "private, no-store");
...
employeeRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    if (role !== "EMPLOYEE")
      throw new HttpError(403, "Employee access required");
    const policy = await pool.query<{ days: number }>(
      `SELECT correction_window_days::int AS days FROM companies WHERE id=$1`,
      [companyId],
    );
    const correctionDays = policy.rows[0]?.days ?? 45;
    if (Date.parse(input.occurredAt) < Date.now() - correctionDays * 86_400_000)
      throw new HttpError(
        400,
        `Corrections are limited to the last ${correctionDays} days`,
      );
    const employeeId = await employeeForUser(sub, companyId);
    const shift = await pool.query<{ id: string }>(
     ...
     ORDER BY abs(extract(epoch FROM starts_at-$3::timestamptz)) LIMIT 1`,
      [companyId, employeeId, input.occurredAt],
    );
    ...
    const result = await pool.query(
     ...
     RETURNING id,status,created_at AS "createdAt",requested_correction AS "requestedCorrection"`,
      [
        ...
        shift.rows[0]?.id ?? null,
        input.reason,
        JSON.stringify(requestedCorrection),
      ],
    );
    await pool.query(
      `INSERT INTO audit_logs(company_id,actor_user_id,action,entity_type,entity_id,next_value)
     VALUES($1,$2,'ATTENDANCE_CORRECTION_REQUESTED','ATTENDANCE_EXCEPTION',$3,$4)`,
      [
        ...
        sub,
        result.rows[0].id,
        JSON.stringify({ ...requestedCorrection, reason: input.reason }),
      ],
    );
...
const leaveRequestSchema = z
  ...
  .superRefine((value, context) => {
    ...
    const duration =
      (Date.parse(value.endsOn) - Date.parse(value.startsOn)) / 86_400_000;
    if (duration > 365)
      context.addIssue({
        code: "custom",
        path: ["endsOn"],
        message: "Leave cannot exceed one year",
      });
  });
...
employeeRouter.post(
  ...
  asyncHandler(async (request, response) => {
    ...
    const overlap = await pool.query(
      `SELECT id FROM leave_requests WHERE company_id=$1 AND employee_id=$2 AND status IN('PENDING','APPROVED') AND starts_on<=$4::date AND ends_on>=$3::date LIMIT 1`,
      [companyId, employeeId, input.startsOn, input.endsOn],
    );
    if (overlap.rows[0])
      throw new HttpError(
        409,
    ...
    const result = await pool.query(
      `INSERT INTO leave_requests(company_id,employee_id,leave_type,starts_on,ends_on,reason) VALUES($1,$2,$3,$4,$5,$6) RETURNING id,leave_type AS "leaveType",starts_on AS "startsOn",ends_on AS "endsOn",reason,status,created_at AS "createdAt"`,
      [
        companyId,
        employeeId,
        input.leaveType,
    ...
    await pool.query(
      `INSERT INTO audit_logs(company_id,actor_user_id,action,entity_type,entity_id,next_value) VALUES($1,$2,'LEAVE_REQUESTED','LEAVE_REQUEST',$3,$4)`,
...
employeeRouter.patch(
  ...
  asyncHandler(async (request, response) => {
    ...
    const previous = await pool.query(
      `SELECT * FROM leave_requests WHERE id=$1 AND company_id=$2 AND employee_id=$3 AND status='PENDING'`,
      [id, companyId, employeeId],
    );
    if (!previous.rows[0])
      throw new HttpError(404, "Pending leave request not found");
    const result = await pool.query(
      `UPDATE leave_requests SET status='CANCELLED',updated_at=now() WHERE id=$1 RETURNING id,status`,
      [id],
    );
    await pool.query(
      `INSERT INTO audit_logs(company_id,actor_user_id,action,entity_type,entity_id,previous_value,next_value) VALUES($1,$2,'LEAVE_CANCELLED','LEAVE_REQUEST',$3,$4,$5)`,
      [
        companyId,
        sub,
        id,
...
const leaveResolutionSchema = z.object({
...
employeeRouter.patch(
  ...
  asyncHandler(async (request, response) => {
    const id = z.string().uuid().parse(request.params.id);
    const input = leaveResolutionSchema.parse(request.body);
    const { companyId, sub } = (request as AuthRequest).auth;
    const client = await pool.connect();
    try {
      await client.query("BEGIN");
      ...
      const result = await client.query(
        `UPDATE leave_requests SET status=$1,manager_note=$2,resolved_by=$3,resolved_at=now(),updated_at=now() WHERE id=$4 RETURNING id,status,manager_note AS "managerNote",resolved_at AS "resolvedAt"`,
        [input.status, input.managerNote, sub, id],
      );
      await client.query(
        `INSERT INTO audit_logs(company_id,actor_user_id,action,entity_type,entity_id,previous_value,next_value) VALUES($1,$2,'LEAVE_RESOLVED','LEAVE_REQUEST',$3,$4,$5)`,
        [
          companyId,
          sub,
          id,
