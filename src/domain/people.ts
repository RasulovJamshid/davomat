...
export function validateEmployee(input: EmployeeInput): EmployeeValidationErrors {
  const errors: EmployeeValidationErrors = {};
  if (input.name.trim().length < 2) errors.name = "Enter the employee’s full name";
  ...
  if (!Number.isFinite(baseSalary) || baseSalary <= 0) errors.baseSalary = "Enter a monthly salary greater than zero";
  if (!Number.isFinite(hourlyRate) || hourlyRate < 0) errors.hourlyRate = "Enter a valid non-negative hourly rate";
  return errors;
}
